...
namespace FindsExplorer.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        {
            [Required]
            [StringLength(100, ErrorMessage = "{0} должно быть минимум {2} символа.", MinimumLength = 3)]
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _emailSender.SendEmailAsync(Input.Email, "Подтвердите вашу электронную почту",
                        $"Для подтверждения адреса электронной почты перейдите по <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>этой ссылке</a>.");
